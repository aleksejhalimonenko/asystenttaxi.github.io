...
function renderService(data) {
  ...
  data.forEach(function(i) {
    try { var y = parseCustomDate(i.date).getFullYear(); if (!isNaN(y) && y>2000) yearsSet[y]=1; } catch(e){}
  });
  ...
  years.forEach(function(y) {
    yearChips += '<div class="chip'+(window._activeSvcYear==y?' active':'')+'" data-year="'+y+'">'+y+'</div>';
  });

  var svcCategories = [
    {key:'all',lbl:'Все'},{key:'плановое',lbl:'Плановое ТО'},{key:'ремонт',lbl:'Ремонт'},
    {key:'расходник',lbl:'Расходники'},{key:'переобувка',lbl:'Переобувка'},
    {key:'покупка',lbl:'Покупка запчастей'},{key:'мойка',lbl:'Мойка'},
  ];
  var catChips = svcCategories.map(function(c){
    return '<div class="chip'+(window._activeSvcCat===c.key?' active':'')+'" data-cat="'+c.key+'">'+c.lbl+'</div>';
  }).join('');

  // Итого за всё время для hero
  ...
  document.querySelectorAll('#svcYearChips .chip').forEach(function(chip){
    chip.addEventListener('click', function(){
      ...
      this.classList.add('active');
      window._activeSvcYear = this.dataset.year;
      _updateSvcStats();
      _applySvcFilters();
    });
  });
  document.querySelectorAll('#svcCatChips .chip').forEach(function(chip){
    chip.addEventListener('click', function(){
      ...
      this.classList.add('active');
      window._activeSvcCat = this.dataset.cat;
      _applySvcFilters();
    });
  });
...
function _updateSvcStats() {
  var data    = window._svcData || [];
  var ccy     = window._svcCcy  || 'zł';
  var yearSel = window._activeSvcYear || 'all';
  var curYear = new Date().getFullYear();
  var curMonth= new Date().getMonth() + 1; // 1–12

  var filtered = yearSel==='all' ? data : data.filter(function(i){
    try{return parseCustomDate(i.date).getFullYear()===parseInt(yearSel);}catch(e){return false;}
  });
  var prevYear = yearSel==='all' ? null : parseInt(yearSel)-1;
  var prevData = prevYear ? data.filter(function(i){
    try{return parseCustomDate(i.date).getFullYear()===prevYear;}catch(e){return false;}
  }) : [];
  ...
  var avgCost   = count ? total/count : 0;

  // В месяц
  var months = 1;
  if (yearSel==='all') {
    var allY={};
    data.forEach(function(i){try{allY[parseCustomDate(i.date).getFullYear()]=1;}catch(e){}});
    months = Math.max(1, Object.keys(allY).length*12);
  } else {
    months = parseInt(yearSel)===curYear ? Math.max(1,curMonth) : 12;
  }
  ...
  if (prevTotal>0 && total>0) {
    var ps=Math.round(((total-prevTotal)/prevTotal)*100);
    trendSumHTML='<span class="'+(ps>0?'trend-up':'trend-down')+'">'+(ps>0?'↑':'↓')+Math.abs(ps)+'%</span> vs '+prevYear;
  } else if (yearSel==='all') { trendSumHTML='<span style="color:var(--text2)">все годы</span>'; }
  if (prevData.length>0 && count>0) {
    var pv=Math.round(((count-prevData.length)/prevData.length)*100);
    trendVisHTML='<span class="'+(pv>0?'trend-up':'trend-down')+'">'+(pv>0?'↑':'↓')+Math.abs(pv)+'%</span> vs '+prevYear;
  }

  // Последний визит + самый дорогой
  ...
  filtered.forEach(function(i){
    try{var d=parseCustomDate(i.date);var k=d.toLocaleDateString('ru-RU',{month:'long',year:'numeric'}).replace(' г.','');mCounts[k]=(mCounts[k]||0)+1;}catch(e){}
  });
  var busyMonth=Object.keys(mCounts).length?Object.entries(mCounts).sort(function(a,b){return b[1]-a[1];})[0]:null;

  // Пробег между визитами
  ...
  var forecastHTML = '';
  var curYearData = data.filter(function(i){try{return parseCustomDate(i.date).getFullYear()===curYear;}catch(e){return false;}});
  var curYearTotal = curYearData.reduce(function(s,i){return s+(parseFloat(i.total)||0);},0);
  if (curMonth < 12 && curYearTotal > 0) {
    var projected = Math.round((curYearTotal / curMonth) * 12);
    var remaining = Math.round(projected - curYearTotal);
    var pctDone   = Math.round((curMonth / 12) * 100);
    forecastHTML =
      '<div class="tco-insight" style="margin-top:10px">'+
        ...
        'Потрачено '+Math.round(curYearTotal).toLocaleString('ru')+' '+ccy+' ('+pctDone+'% года прошло). '+
        'Ещё осталось потратить ~<b>'+remaining.toLocaleString('ru')+' '+ccy+'</b>.</span>'+
      '</div>';
  }
  ...
  var html =
    ...
    // График — только за выбранный год, или последние 12 мес если "Все"
    '<div class="slbl" style="margin-top:4px">Расходы по месяцам</div>'+
    '<div class="group" style="padding:16px 12px">'+_buildSvcBarChart(filtered, yearSel, ccy)+'</div>'+

    // Доnat
    (distRows.length?
      '<div class="slbl">Распределение расходов</div>'+
      '<div class="group" style="padding:16px">'+_buildSvcDonut(distRows,catTotal,ccy)+'</div>':'')+

    // Год к году
    (allYearsArr.length>1?
      '<div class="slbl">Год к году</div>'+
      '<div class="group" style="padding:0">'+_buildYearCompare(allYearsArr,ccy)+'</div>':'')+
...
function _getMostCommonType(data) {
  var counts={};
  data.forEach(function(i){var t=(i.type||'другое').toLowerCase().trim();counts[t]=(counts[t]||0)+1;});
  var top=Object.entries(counts).sort(function(a,b){return b[1]-a[1];})[0];
  return top?top[0].toUpperCase():'—';
}

// Столбчатый SVG-график — умный: если год выбран, показывает 12 мес; если "Все" — последние 12 мес
function _buildSvcBarChart(data, yearSel, ccy) {
  ...
  data.forEach(function(i){
    try{
      ...
      if(!monthly[key]) monthly[key]={lbl:lbl,total:0,year:d.getFullYear(),month:d.getMonth()+1};
  ...
  // Если "Все" и данных > 18 месяцев — берём только последние 18
  ...
  var labels=keys.map(function(k){return monthly[k].lbl;});
  var maxVal=Math.max.apply(null,vals)||1;
  var n=keys.length;

  // Фиксированная высота графика — 120px рабочей зоны
  var H=150, chartH=100, topPad=20, botPad=30;
  // Ширина: минимум 28px на столбец + отступы
  var BAR_W=Math.max(18, Math.min(36, Math.floor(280/n)-4));
  var GAP=Math.max(3,Math.floor(BAR_W*0.3));
  var totalW=Math.max(280,(BAR_W+GAP)*n+GAP);

  var bars='', xlbls='', ylines='';

  // Линии сетки Y (3 штуки)
  [0.25,0.5,0.75,1].forEach(function(f){
    var y=topPad+chartH*(1-f);
    var val=Math.round(maxVal*f);
    ylines+='<line x1="0" y1="'+y+'" x2="'+totalW+'" y2="'+y+'" stroke="var(--sep)" stroke-width="0.5"/>';
    if(f===1) ylines+='<text x="2" y="'+(y+10)+'" font-size="9" fill="var(--text2)" font-family="-apple-system">'+val.toLocaleString('ru')+' '+ccy+'</text>';
  });
  ...
  keys.forEach(function(k,i){
    ...
    var bh=Math.max(3,Math.round((v/maxVal)*chartH));
    var x=GAP+i*(BAR_W+GAP);
    var y=topPad+chartH-bh;
    // Текущий месяц — акцент, максимальный — тоже акцент
    var isCur=(monthly[k].year===curYear && monthly[k].month===curMonth);
    var isMax=(v===maxVal);
    var color=isCur?'var(--green)':(isMax?'var(--accent)':'var(--accent-bg)');
    var stroke=isCur?'stroke="var(--green)" stroke-width="1.5"':isMax?'stroke="var(--accent)" stroke-width="1"':'';

    bars+='<rect x="'+x+'" y="'+y+'" width="'+BAR_W+'" height="'+bh+'" rx="4" fill="'+color+'" '+stroke+'/>';

    // Подпись суммы над максимальным столбцом
    if(isMax||isCur){
      bars+='<text x="'+(x+BAR_W/2)+'" y="'+(y-4)+'" text-anchor="middle" font-size="9" fill="'+(isCur?'var(--green)':'var(--accent)')+'" font-family="-apple-system" font-weight="600">'+Math.round(v).toLocaleString('ru')+'</text>';
    }

    // Подписи месяцев — все если <=12, через один если больше
    var showLbl=(n<=12)||(i%2===0);
    if(showLbl){
      xlbls+='<text x="'+(x+BAR_W/2)+'" y="'+(H-6)+'" text-anchor="middle" font-size="9" fill="var(--text2)" font-family="-apple-system">'+labels[i]+'</text>';
    }
  });

  return '<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;margin:0 -4px">'+
    '<svg width="'+totalW+'" height="'+H+'" viewBox="0 0 '+totalW+' '+H+'" style="display:block;min-width:100%">'+
      ylines+bars+xlbls+
    '</svg></div>';
}
...
function _buildSvcDonut(distRows, total, ccy) {
  ...
  var top=distRows[0];
  var centre='<text x="'+OX+'" y="'+(OY-6)+'" text-anchor="middle" font-size="15" font-weight="700" fill="var(--text)" font-family="-apple-system">'+top.pct+'%</text>'+
    '<text x="'+OX+'" y="'+(OY+12)+'" text-anchor="middle" font-size="10" fill="var(--text2)" font-family="-apple-system">'+top.lbl+'</text>';
  var rows='';
  distRows.forEach(function(d,i){
    rows+='<div style="display:flex;align-items:center;gap:8px;'+(i<distRows.length-1?'margin-bottom:9px;':'')+'">'+
      '<div style="width:9px;height:9px;border-radius:3px;flex-shrink:0;background:'+d.color+'"></div>'+
      '<span style="font-size:13px;color:var(--text);flex:1">'+d.lbl+'</span>'+
      '<span style="font-size:12px;color:var(--text2);margin-right:6px">'+Math.round(d.value).toLocaleString('ru')+' '+ccy+'</span>'+
      '<span style="font-size:13px;font-weight:600;color:'+d.color+';min-width:32px;text-align:right">'+d.pct+'%</span>'+
    '</div>';
  });
...
function _buildYearCompare(yearsArr, ccy) {
  var maxTotal=Math.max.apply(null,yearsArr.map(function(y){return y.total;}))||1;
  var html='';
  yearsArr.slice().reverse().forEach(function(y){
    var bw=Math.round((y.total/maxTotal)*100);
    html+='<div style="display:flex;align-items:center;padding:11px 16px;gap:12px;border-bottom:0.5px solid var(--sep)">'+
      ...
      '<div style="flex:1">'+
        '<div style="height:5px;background:var(--bg2);border-radius:3px;overflow:hidden"><div style="height:100%;width:'+bw+'%;background:var(--indigo);border-radius:3px"></div></div>'+
        '<div style="display:flex;justify-content:space-between;margin-top:4px">'+
          '<span style="font-size:11px;color:var(--text2)">'+y.count+' визит.</span>'+
          '<span style="font-size:11px;color:var(--text2)">ср. '+Math.round(y.avg).toLocaleString('ru')+' '+ccy+'</span>'+
        '</div>'+
      '</div>'+
      '<div style="font-size:15px;font-weight:600;color:var(--text);min-width:80px;text-align:right">'+Math.round(y.total).toLocaleString('ru')+' '+ccy+'</div>'+
    '</div>';
  });
  return html;
}
...
function buildTimelineHTML(list) {
  ...
  Object.keys(grp).forEach(function(month){
    ...
    grp[month].forEach(function(s){
      var icon=getServiceIconSVG(s.type);
      html+='<div class="tl-row">'+
        '<div class="tl-icon '+icon.cls+'">'+icon.svg+'</div>'+
        '<div class="tl-body">'+
          '<div class="tl-title">'+(s.description||s.type||'Обслуживание')+'</div>'+
          '<div class="tl-meta">'+(s.date||'—')+' · '+(s.mileage||'—')+' км · '+(s.type||'')+'</div>'+
        '</div>'+
        '<div class="tl-price">'+(s.total?s.total+' '+(window._svcCcy||'zł'):'—')+'</div>'+
      '</div>';
    });
    html+='</div>';
  });
...
function _applyHistoryFilters() {
  ...
  const el=document.getElementById('fuelTimeline');
...
function _applySvcFilters() {
  ...
  var el=document.getElementById('svcTimeline');
